...
export default function AdminPage({ }: AdminPageProps) {
    ...
    const changeBookingStatus = async (id: string, status: BookingStatus) => {
        await api.AdminApi.changeBookingStatus(id, status);
        await loadBookings();
    }

    if (!user.hasRole(Role.admin)) return <div>{t('pages.admin.noaccess')}</div>
    ...
    return (<Grid container justify='center'>
        <Grid className={classes.root} item xs={12} md={8} lg={6}>
            ...
            >
                <Tab label={t('pages.admin.tabs.users')} icon={<Badge badgeContent={users.length} color="primary"><PeopleIcon /></Badge>} id='0' />
                <Tab label={t('pages.admin.tabs.bookings')} icon={<Badge badgeContent={bookings.length} color="primary"><EventIcon /></Badge>} id='1' />
                <Tab label={t('pages.admin.tabs.account')} icon={<AccountBalanceIcon />} id='2' />
            </Tabs>
